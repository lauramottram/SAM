	...
	public function generate_css( $post_id, $panels_data = false, $layout_data = false ) {
		...
		foreach ( $layout_data as $ri => $row ) {
			...
			foreach ( $row['cells'] as $ci => $cell ) {
				...
				$rounded_width = round( $cell['weight'] * ( 100 - ( ($cell_count - 1 ) * 2.6 ) ), 3 ) . '%'; // TUT Customization
				$calc_width = 'calc(' . round( $cell['weight'] * ( 100 - ( ($cell_count - 1 ) * 2.6 ) ), 3 ) . '% )'; // TUT Customization
...
// START - TUT Customization - Remove responsive design
...
//		$this->render_element( 'div', $layout_attributes ); // TUT Customization
