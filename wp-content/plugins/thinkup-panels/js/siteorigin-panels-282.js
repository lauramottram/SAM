...
module.exports = panels.view.dialog.extend( {
	...
	render: function () {
		...
		this.$('.so-title .widget-name').closest('div[class^="so-panels-dialog"]').find('.so-content').first().addClass( 'thinkup-content-' + this.model.getWidgetField( 'id' ) ); // TUT Customization - allows id to be added to so-content class to style ThinkUpThemes Builder widgets. 
			...
			$s.find( '.so-image-selector' ).click( function ( e ) {
				...
				if ( frame === null ) {
					...
					frame.on( 'select', function () {
						...
						// Store the ID
						$s.find( '.so-image-selector > input' ).val( attachment.url )
