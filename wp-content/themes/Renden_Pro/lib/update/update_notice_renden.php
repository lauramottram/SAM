<?php
/**
 * ThinkUpThemes.com Upgrade notice - Renden Pro Theme
 * 
...
function thinkup_latest_theme_version($interval) {
	...
	$renden_notifier_file_url = 'http://thinkupcloud.com/cloudfiles/thinkupthemes/updates/themes/update_' . str_replace( ' pro', '', strtolower( $theme_data['Name'] ) ) . '.xml'; // Set to ThinkUpCloud - replace with CDN source in future.

	// Check if Dropbox link is broken
	if( function_exists('curl_init') ) { // if cURL is available, use it...
		$dropbox_ch = curl_init($renden_notifier_file_url);
		...
		$dropbox_check = @file_get_contents($renden_notifier_file_url); // ...if not, use the common file_get_contents()
	}

	// If dropbox is empty then check thinkupcloud.com
	if( empty( $dropbox_check ) ) {
		$renden_notifier_file_url = 'http://thinkupcloud.com/cloudfiles/thinkupthemes/updates/themes/update_' . str_replace( ' pro', '', strtolower( $theme_data['Name'] ) ) . '.xml';
	}

	$renden_db_cache_field = 'renden_contempo-notifier-cache';
	$renden_db_cache_field_last_updated = 'renden_contempo-notifier-last-updated';
	$renden_last = get_option( $renden_db_cache_field_last_updated );
	$renden_now = time();
	// check the cache
	if ( !$renden_last || (( $renden_now - $renden_last ) > $interval) ) {
		// cache doesn't exist, or is old, so refresh it
		if( function_exists('curl_init') ) { // if cURL is available, use it...
			$renden_ch = curl_init($renden_notifier_file_url);
			curl_setopt($renden_ch, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($renden_ch, CURLOPT_HEADER, 0);
			curl_setopt($renden_ch, CURLOPT_TIMEOUT, 10);
			$renden_cache = curl_exec($renden_ch);
			curl_close($renden_ch);
		} else {
			$renden_cache = @file_get_contents($renden_notifier_file_url); // ...if not, use the common file_get_contents()
		}
		
		if ($renden_cache) {			
			// we got good results
			update_option( $renden_db_cache_field, $renden_cache );
			update_option( $renden_db_cache_field_last_updated, time() );			
		}
		// read from the cache file
		$renden_notifier_data = get_option( $renden_db_cache_field );
	}
	else {
	...
	if ( strpos( $renden_notifier_data,'looking for. Check out our' ) !== false) {
	} else {
		@$renden_xml = simplexml_load_string($renden_notifier_data); 
	}
	return $renden_xml;
}
