...
if ( ! class_exists( 'thinkup_widget_flickr' ) ) {
	class thinkup_widget_flickr extends WP_Widget {
		...
		function widget($args, $instance) {
			...
			echo $before_widget;
			$title = empty($instance['title']) ? __( 'Flickr Feed', 'renden') : apply_filters('widget_title', $instance['title']);
			if (!empty($title))
			...
			if($flickrphoto->photos->photo) {
				...
				echo '<div class="error-icon">' . __( 'Oops! There&#39;s been an error!', 'renden') . '<br />' .  __( 'Check that the Flickr ID is correct at', 'renden') . '<a href="http://idgettr.com/">http://idgettr.com/</a>.</div>';
				}
